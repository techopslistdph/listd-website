...
interface ErrorResponse {
  success: false
  error: {
    code: 'VALIDATION_ERROR' | 'UNAUTHORIZED' | 'FORBIDDEN' | 'NOT_FOUND' | 'DUPLICATE_RECORD' | 'INTERNAL_ERROR'
    message: string
...
const ListingTypeApi = {
  getLists: async (): Promise<List[]> => {
    try {
      const response = await api.get<{ success: true; data: List[] } | ErrorResponse>('/api/listing-types')
  ...
  getList: async (id: string): Promise<List> => {
    try {
      const response = await api.get<{ success: true; data: List } | ErrorResponse>(`/api/listing-types/${id}`)
      if ('error' in response) {
        throw new Error(response.error.message)
      }
      return response.data
    } catch (error) {
      throw error instanceof Error ? error : new Error(`Failed to fetch list ${id}`)
    }
  },

  createList: async (data: CreateListingTypeData): Promise<List> => {
    try {
      const response = await api.post<{ success: true; data: List } | ErrorResponse>('/api/listing-types', data)
      if ('error' in response) {
        throw new Error(response.error.message)
      }
      return response.data
  ...
  updateList: async (data: UpdateListingTypeData): Promise<List> => {
    try {
      ...
      const response = await api.put<{ success: true; data: List } | ErrorResponse>(`/api/listing-types/${id}`, updateData)
      if ('error' in response) {
        throw new Error(response.error.message)
      }
      return response.data
    } catch (error) {
      throw error instanceof Error ? error : new Error(`Failed to update list ${data.id}`)
    }
  },

  deleteList: async (id: DeleteListingTypeData): Promise<void> => {
    try {
      const response = await api.delete<{ success: true } | ErrorResponse>(`/api/listing-types/${id}`)
      if ('error' in response) {
        throw new Error(response.error.message)
      }
    } catch (error) {
      throw error instanceof Error ? error : new Error(`Failed to delete list ${id}`)
    }
  },
}
...
export function useDeleteListingType() {
  ...
  return useMutation({
    ...
    onSuccess: (_, deletedId: DeleteListingTypeData) => {
      queryClient.removeQueries({ queryKey: queryKeys.listingTypes.detail(deletedId.id) })
      queryClient.invalidateQueries({ queryKey: queryKeys.listingTypes.all })
    },
  })
} 